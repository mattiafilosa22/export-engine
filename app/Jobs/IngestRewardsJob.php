...
/**
 * Ingests a rewards batch (append, idempotent). Direct alternative to the
 * event-driven `reward_granted` event type: rows inserted here have `event_id =
 ...
 */
class IngestRewardsJob extends AbstractIngestJob
{
    /**
    ...
    private function ingestChunk(
        ...
        foreach ($chunk as $row) {
            ...
            $rows[] = [
                ...
                'status' => isset($row['status']) ? (string) $row['status'] : Reward::STATUS_GRANTED,
                'granted_at' => $normalizer->toUtc($row['granted_at']),
                'redeemed_at' => isset($row['redeemed_at']) ? $normalizer->toUtc($row['redeemed_at']) : null,
        ...
        $affected = Reward::insertOrIgnore($rows);
